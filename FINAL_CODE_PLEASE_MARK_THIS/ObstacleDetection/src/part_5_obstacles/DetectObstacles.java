...
public class DetectObstacles 
{
	...
	private Graph graph;
	private LocaliseGeoff localise;
	
	/**constructor will initialise attributes
	 * 
	 */
	public DetectObstacles(GeoffSetUp geoff, IGridMap grid, Graph graph, LocaliseGeoff localise)
	{
		this.geoff = geoff;
		this.grid = grid;
		this.graph = graph;
	...
	{
		double currentDistance = geoff.getOpticalDistance();//the distance from the optical distance sensor
	...
	private void reRoute(int xOld, int yOld, int xNew, int yNew, JunctionBehavior junction)
	{
		...
		{
			newPath = this.localise.getSearch().getListOfActions(this.graph.nodeWith(new Coord(xOld, yOld)),this.localise.getSearch().getGoal());
		}
		else if(destinationsLeft == 2)
		{
			newPath = this.localise.getSearch().getListOfActions(this.graph.nodeWith(new Coord(xOld, yOld)), this.localise.getSearch().getInter()).append(
						this.localise.getSearch().getListOfActions(this.localise.getSearch().getInter(), this.localise.getSearch().getGoal()));
		}
